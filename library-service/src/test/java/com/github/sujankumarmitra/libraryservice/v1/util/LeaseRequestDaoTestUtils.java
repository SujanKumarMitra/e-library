    ...
    public static Mono<R2dbcLeaseRequest> insertLeaseRequest(ConnectionAccessor connAccessor, UUID bookId) {
        ...
        return connAccessor.inConnectionMany(connection ->
                        Flux.from(connection
                                .createStatement("INSERT INTO lease_requests(library_id, book_id,user_id,status,timestamp) VALUES ($1,$2,$3,$4,$5) RETURNING id")
                                .bind("$1", leaseRequest.getLibraryId())
                                .bind("$2", leaseRequest.getBookUuid())
                                .bind("$3", leaseRequest.getUserId())
                                .bind("$4", leaseRequest.getStatus().toString())
                                .bind("$5", leaseRequest.getTimestamp())
                                .execute()))
